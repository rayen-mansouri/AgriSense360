...
class GenerateEntitiesCommand extends Command
{
    ...
    private function generateEntityCode(
        ...
        foreach ($ownFks as $fk) {
            $refClass = $this->tableToClassName($fk['refTable']);
            $prop = lcfirst($refClass);
            $ucProp = ucfirst($prop);
            $c .= "\n    public function get$ucProp(): ?$refClass { return \$this->$prop; }\n";
            $c .= "    public function set$ucProp(?$refClass \$$prop): static { \$this->$prop = \$$prop; return \$this; }\n";
        }
        foreach ($allTables as $ot) {
            if (!isset($fkMap[$ot]) || isset($m2mTables[$ot])) continue;
            foreach ($fkMap[$ot] as $fk) {
                if ($fk['refTable'] !== $table) continue;
                $otherClass = $this->tableToClassName($ot);
                $isO2Oinv   = isset($uniqueMap[$ot][$fk['column']]);
                if ($isO2Oinv) {
                    $prop = lcfirst($otherClass);
                    $c .= "\n    public function get{$otherClass}(): ?$otherClass { return \$this->$prop; }\n";
                    $c .= "    public function set{$otherClass}(?$otherClass \$$prop): static { \$this->$prop = \$$prop; return \$this; }\n";
                } else {
                    $cv  = lcfirst($otherClass).'s'; $sv = lcfirst($otherClass);
                    $ucv = ucfirst($cv); $ucs = ucfirst($sv);
                    $ucCls = ucfirst(lcfirst($className));
                    $c .= "\n    /** @return Collection<int, $otherClass> */\n";
                    $c .= "    public function get$ucv(): Collection { return \$this->$cv; }\n";
                    $c .= "    public function add$ucs($otherClass \$$sv): static {\n";
                    $c .= "        if (!\$this->{$cv}->contains(\$$sv)) { \$this->{$cv}->add(\$$sv); \$$sv->set$ucCls(\$this); }\n";
                    $c .= "        return \$this;\n    }\n";
                    $c .= "    public function remove$ucs($otherClass \$$sv): static {\n";
                    $c .= "        if (\$this->{$cv}->removeElement(\$$sv) && \$$sv->get$ucCls() === \$this) { \$$sv->set$ucCls(null); }\n";
                    $c .= "        return \$this;\n    }\n";
                }
            }
        }
