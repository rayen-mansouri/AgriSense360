...
public function edit(
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        if ($photoFile) {
            ...
            if ($produit->getPhoto()) {
                $oldPhotoPath = $this->getParameter('produits_directory') . '/' . $produit->getPhoto();
        ...
        // Gérer le stock
        $stockData = [
            'quantite' => $request->request->get('stock_quantite'),
            'unite' => $request->request->get('stock_unite'),
            'seuil' => $request->request->get('stock_seuil'),
            'emplacement' => $request->request->get('stock_emplacement'),
            'date_reception' => $request->request->get('stock_date_reception'),
            'date_expiration' => $request->request->get('stock_date_expiration'),
        ];
        
        if ($stockData['quantite'] > 0 && $stockData['unite']) {
            $stock = $produit->getStock() ?? new Stock();
            $stock->setProduit($produit);
            $stock->setQuantite((float) $stockData['quantite']);
            $stock->setUnite($stockData['unite']);
            $stock->setSeuilAlerte($stockData['seuil'] ? (float) $stockData['seuil'] : 0);
            $stock->setEmplacement($stockData['emplacement']);
            
            if ($stockData['date_reception']) {
                $stock->setDateReception(new \DateTime($stockData['date_reception']));
            }
            if ($stockData['date_expiration']) {
                $stock->setDateExpiration(new \DateTime($stockData['date_expiration']));
            }
