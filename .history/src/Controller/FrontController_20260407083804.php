...
public function edit(
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        if ($stockQuantite && (float)$stockQuantite > 0 && $stockUnite) {
            ...
            $em->persist($stock);
            // Dans front_produit_edit
$stock->setSeuilAlerte($request->request->get('stock_seuil'));
$stock->setEmplacement($request->request->get('stock_emplacement'));

if ($request->request->get('stock_date_reception')) {
    $stock->setDateReception(new \DateTime($request->request->get('stock_date_reception')));
}
if ($request->request->get('stock_date_expiration')) {
    $stock->setDateExpiration(new \DateTime($request->request->get('stock_date_expiration')));
}
        }
...
#[Route('/stock/modifier/{id}', name: 'front_stock_edit', methods: ['GET', 'POST'])]
public function editStock(
    Stock $stock
): Response {
    $produit = $stock->getProduit();
    
    // Rediriger vers l'édition du produit
    return $this->redirectToRoute('front_produit_edit', ['id' => $produit->getId()]);
}
