...
public function dashboard(Request $request): Response
{
    ...
    // ✅ CORRECTION 1: Utiliser la version paginée
    $produits = $this->produitRepo->findFilteredPaginated($search, $catFilter, $pSort, $pDir, $limit, $page);
    $stocks   = $this->stockRepo->findFiltered($search, $sSort, $sDir);

    $alertes    = $this->stockRepo->findAlertes();
    
    // ✅ CORRECTION 2: Ajouter une limite aux catégories
    $categories = $this->produitRepo->findAllCategories(50);
    ...
    return $this->render('dashboard/index.html.twig', [
        ...
        // ✅ CORRECTION 3: Utiliser count() optimisé
        'totalProduits' => $this->produitRepo->countProducts(),
        'totalStocks'   => $this->stockRepo->countStocks(),
...
public function searchByBarcode(Request $request, ProduitRepository $produitRepo): JsonResponse
{
    ...
    return $this->json([
        ...
        'url' => $this->generateUrl('front_produit_show', ['id' => $produit->getId()])  // ← front_produit_show
    ]);
}
#[Route('/scannerback', name: 'produit_scanner', methods: ['GET'])]
...
#[Route('/export/excel/stocks', name: 'export_excel_stocks')]
public function exportExcelStocks(ExcelExportService $excelService): Response
{
    return $excelService->exportStocks();
}

#[Route('/export/excel/produits', name: 'export_excel_produits')]
public function exportExcelProduits(ExcelExportService $excelService): Response
{
    return $excelService->exportProducts();
}


#[Route('/exchangeback', name: 'api_exchange_view')]
...
#[Route('/commodityback', name: 'api_commodity_view')]
public function commodityView(ProduitRepository $produitRepo): Response
{
    // Récupérer TOUS les produits
    $produits = $produitRepo->findFiltered('', '', 'nom', 'asc');
    if (count($produits) > 100) {
        $produits = array_slice($produits, 0, 100);
    }
   ...
   $categoriesComparables = ['Semences', 'Engrais', 'Alimentation animale', 'Céréales'];
    $produitsCompatibles = $produitRepo->findBy(['categorie' => $categoriesComparables], null, 50);
...
public function completeAnalysis(ProduitRepository $produitRepo): Response
{
    ...
    if (count($produits) > 200) {
        $produits = array_slice($produits, 0, 200);
    }
    ...
    foreach ($produits as $produit) {
        ...
        if ($produit->getPhotoUrl() !== null && $produit->getPhotoUrl() !== '') {
            $score += 20;
        }
        if ($produit->getSku() !== null && $produit->getSku() !== '') {
            $score += 10;
        }
        
        $qualites[] = [
            'produit' => $produit->getNom() ?? 'Sans nom',
            'score_global' => min(100, $score),
            'niveau' => $score >= 70 ? '✅ Bonne qualité' : '⚠️ Qualité moyenne',
            'champs_manquants' => []
        ];
        
        // Calcul rentabilité
        $prix = $produit->getPrixUnitaire() ?? 0;
        $totalPrix += $prix;
        
        $rentabilite[] = [
            'produit' => $produit->getNom() ?? 'Sans nom',
            'prix_vente' => $prix,
            'marge_brute' => round($prix * 0.3, 2),
            'taux_marge' => 30,
            'performance' => $prix > 50 ? '🚀 Très rentable' : '✅ Rentable'
        ];
    }
    
    $prixMoyen = $nbProduits > 0 ? round($totalPrix / $nbProduits, 2) : 0;
...
{
    $produits = $produitRepo->findFiltered('', '', 'nom', 'asc');
    $categories = $produitRepo->findAllCategories(50);
...
public function classify(Request $request, ProduitRepository $produitRepo): JsonResponse
{
    ...
    $produits = $produitRepo->findFiltered('', '', 'nom', 'asc');
    if (count($produits) > 500) {
        $produits = array_slice($produits, 0, 500);
    }
    ...
    /** @var array<array{distance: float, categorie: string}> $distances */
