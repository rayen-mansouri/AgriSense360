...
{
    private const ANALYTICS_BATCH_SIZE = 200;

    #[Route('/analytics/predict', name: 'analytics_predict', methods: ['POST'])]
    public function predictAnalytics(Request $request, EntityManagerInterface $em, AiPredictionService $aiPredictionService): RedirectResponse
    {
    ...
    public function predictAllAnalytics(Request $request, EntityManagerInterface $em, AiPredictionService $aiPredictionService): RedirectResponse
    {
        $model = $aiPredictionService->normalizeModel((string) $request->request->get('model', 'general'));
        $state = $this->getAnalyticsState($request);
        $state['selectedModel'] = $model;
        ...
        $batchResults = [];
        foreach ($this->getAnimalBatches($em) as $animalsBatch) {
            $batchResults = array_merge(
                $batchResults,
                $aiPredictionService->analyzeAll(
                    $animalsBatch,
    ...
    public function sendAnalyticsVetReport(Request $request, EntityManagerInterface $em, AiPredictionService $aiPredictionService, BrevoMailService $brevoMailService): RedirectResponse
    {
        $model = $aiPredictionService->normalizeModel((string) $request->request->get('model', 'general'));
    ...
    private function getAnimalBatches(EntityManagerInterface $em): \Generator
    {
        $offset = 0;
        while (true) {
            $batch = $em->getRepository(Animal::class)->createQueryBuilder('a')
                ->orderBy('a.id', 'ASC')
                ->setFirstResult($offset)
                ->setMaxResults(self::ANALYTICS_BATCH_SIZE)
                ->getQuery()
                ->getResult();
            ...
            yield $batch;
            $offset += self::ANALYTICS_BATCH_SIZE;
        }
    }
}
