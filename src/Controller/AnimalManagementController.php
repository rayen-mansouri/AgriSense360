...
{
    private const PREDICTOR_ANIMAL_LIMIT = 500;

    #[Route('/animal-management', name: 'animal_management_index', methods: ['GET'])]
    public function index(
        ...
        if ($animalSearchTerm !== '') {
            ...
            if ($animalSearchField === 'all') {
                $animalQb
                    ->andWhere('LOWER(CONCAT(\'\', a.id)) LIKE :term OR LOWER(CONCAT(\'\', a.ear_tag)) LIKE :term OR LOWER(a.type) LIKE :term OR LOWER(CONCAT(\'\', a.weight)) LIKE :term OR LOWER(a.health_status) LIKE :term OR LOWER(CONCAT(\'\', a.birth_date)) LIKE :term OR LOWER(a.origin) LIKE :term OR LOWER(a.location) LIKE :term')
                    ->setParameter('term', $term);
            } else {
                $this->applyAnimalFieldSearch($animalQb, $animalSearchField, $term);
            }
        }
        ...
        $animals = $animalQb
            ->orderBy($animalSortMap[$animalSort], $animalDir)
        ...
        if ($currentTab === 'dossiers' && $visitVisit && $visitLocationParam !== '') {
            ...
            if ($visitTotal === 0) {
                ...
                $visitLinkParams = ['visit' => 1, 'visitLocation' => $visitLocationParam, 'visitIndex' => $visitIndex];
            }
        }
        ...
        if ($currentTab === 'predictor') {
            $analyticsAnimals = $em->getRepository(Animal::class)->createQueryBuilder('a')
                ->orderBy('a.id', 'ASC')
                ->setMaxResults(self::PREDICTOR_ANIMAL_LIMIT)
                ->getQuery()
                ->getResult();
        }
        ...
        if ($recordSearchTerm !== '') {
            ...
            if ($recordSearchField === 'all') {
                $recordQb
                    ->andWhere('LOWER(CONCAT(\'\', r.id)) LIKE :rterm OR LOWER(CONCAT(\'\', r.record_date)) LIKE :rterm OR LOWER(CONCAT(\'\', r.weight)) LIKE :rterm OR LOWER(r.appetite) LIKE :rterm OR LOWER(r.condition_status) LIKE :rterm OR LOWER(CONCAT(\'\', COALESCE(r.milk_yield, r.wool_length, r.egg_count, 0))) LIKE :rterm OR LOWER(r.notes) LIKE :rterm')
                    ->setParameter('rterm', $term);
            } elseif ($recordSearchField === 'production') {
                $recordQb
                    ->andWhere('LOWER(CONCAT(\'\', COALESCE(r.milk_yield, r.wool_length, r.egg_count, 0))) LIKE :rterm')
                    ->setParameter('rterm', $term);
            } else {
                $this->applyRecordFieldSearch($recordQb, $recordSearchField, $term);
            }
        }

        if ($recordSort === 'production') {
            $recordQb->orderBy('COALESCE(r.milk_yield, r.wool_length, r.egg_count, 0)', $recordDir);
        } else {
            $recordQb->orderBy($recordSortMap[$recordSort], $recordDir);
        }
    ...
    private function buildAnalyticsDashboard(EntityManagerInterface $em, ChartBuilderInterface $chartBuilder): array
    {
        ...
        $fallbackConditionColors = ['#5a9814', '#f57c00', '#1e88e5', '#e53935', '#8e24aa', '#00897b', '#6d4c41'];
        $colorIndex = 0;
        foreach ($conditionCounts as $label => $value) {
            ...
            $conditionColors[] = $conditionPalette[$label] ?? $fallbackConditionColors[$colorIndex++ % count($fallbackConditionColors)];
        }
        ...
        $conditionChart->setOptions(['responsive' => true, 'maintainAspectRatio' => false]);
        ...
        $typeChart->setOptions(['responsive' => true, 'maintainAspectRatio' => false]);
    ...
    {
        return [
            'summary' => ['totalAnimals' => 0, 'totalRecords' => 0, 'vaccinationRate' => 0, 'atRiskCount' => 0],
    ...
    private function applyAnimalFieldSearch(QueryBuilder $animalQb, string $animalSearchField, string $term): void
    {
        if ($animalSearchField === 'id') {
            $animalQb->andWhere('LOWER(CONCAT(\'\', a.id)) LIKE :term')->setParameter('term', $term);
            return;
        }
        if ($animalSearchField === 'earTag') {
            $animalQb->andWhere('LOWER(CONCAT(\'\', a.ear_tag)) LIKE :term')->setParameter('term', $term);
            return;
        }
        if ($animalSearchField === 'type') {
            $animalQb->andWhere('LOWER(a.type) LIKE :term')->setParameter('term', $term);
            return;
        }
        if ($animalSearchField === 'weight') {
            $animalQb->andWhere('LOWER(CONCAT(\'\', a.weight)) LIKE :term')->setParameter('term', $term);
            return;
        }
        if ($animalSearchField === 'healthStatus') {
            $animalQb->andWhere('LOWER(a.health_status) LIKE :term')->setParameter('term', $term);
            return;
        }
        if ($animalSearchField === 'birthDate') {
            $animalQb->andWhere('LOWER(CONCAT(\'\', a.birth_date)) LIKE :term')->setParameter('term', $term);
            return;
        }
        if ($animalSearchField === 'origin') {
            $animalQb->andWhere('LOWER(a.origin) LIKE :term')->setParameter('term', $term);
            return;
        }
        if ($animalSearchField === 'location') {
            $animalQb->andWhere('LOWER(a.location) LIKE :term')->setParameter('term', $term);
        }
    }

    private function applyRecordFieldSearch(QueryBuilder $recordQb, string $recordSearchField, string $term): void
    {
        if ($recordSearchField === 'id') {
            $recordQb->andWhere('LOWER(CONCAT(\'\', r.id)) LIKE :rterm')->setParameter('rterm', $term);
            return;
        }
        if ($recordSearchField === 'recordDate') {
            $recordQb->andWhere('LOWER(CONCAT(\'\', r.record_date)) LIKE :rterm')->setParameter('rterm', $term);
            return;
        }
        if ($recordSearchField === 'weight') {
            $recordQb->andWhere('LOWER(CONCAT(\'\', r.weight)) LIKE :rterm')->setParameter('rterm', $term);
            return;
        }
        if ($recordSearchField === 'appetite') {
            $recordQb->andWhere('LOWER(r.appetite) LIKE :rterm')->setParameter('rterm', $term);
            return;
        }
        if ($recordSearchField === 'conditionStatus') {
            $recordQb->andWhere('LOWER(r.condition_status) LIKE :rterm')->setParameter('rterm', $term);
            return;
        }
        if ($recordSearchField === 'notes') {
            $recordQb->andWhere('LOWER(r.notes) LIKE :rterm')->setParameter('rterm', $term);
        }
    }
}
