...
class OptionController extends AbstractController
{
    ...
    public function trainAiModel(AiTrainingService $aiTrainingService): RedirectResponse
    {
        try {
            ...
            $this->addFlash('success', 'Modele entraine. Redemarrez le serveur IA (api.py) pour appliquer.');
        } catch (\Throwable $e) {
    ...
    public function sendVetReport(Request $request, EntityManagerInterface $em, BrevoMailService $brevoMail, VeterinaryReportEmailContentBuilder $reportBuilder): RedirectResponse
    {
        ...
        $atRisk = $em->getRepository(Animal::class)->createQueryBuilder('a')
            ->where('LOWER(a.health_status) IN (:statuses)')
    ...
    public function exportFarmReportPdf(Request $request, FarmPdfReportService $farmPdfReportService): Response
    {
        ...
        return $farmPdfReportService->buildResponse($includeSummary, $includeAllAnimals, $includeAtRisk, $includeRecentRecords);
    }
}
